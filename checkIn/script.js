...
(function (WeatherType) {
    WeatherType["SummerTime"] = "SummerTime";
...
const LogInUtility = {
    verify: async (pin, isCheckIn = true) => {
        return new Promise((resolve, reject) => {
            setTimeout(async () => {
                try {
                    const result = isCheckIn ? await signIn(pin) : await signOut(pin);
                    if (result) {
                        resolve(true);
                    } else {
                        reject(isCheckIn ? 'Check-in failed' : 'Check-out failed');
                    }
                } catch (error) {
                    reject(error);
                }
...
const Pin = () => {
    ...
    React.useEffect(() => {
        if (userStatus === UserStatus.LoggingIn || userStatus === UserStatus.LogInError || userStatus === UserStatus.LoggedIn) {
            ref.current.focus();
        } else {
            setPinTo("");
        }
        ...
        if (pin.length === 4) {
            const verify = async () => {
                try {
                    ...
                    const isCheckIn = userStatus !== UserStatus.LoggedIn;
                    if (await LogInUtility.verify(pin, isCheckIn)) {
                        setUserStatusTo(isCheckIn ? UserStatus.LoggedIn : UserStatus.LoggedOut);
                    }
                } catch (err) {
                    console.error(err);
    ...
    return (
        React.createElement("div", { id: "app-pin-wrapper" },
            React.createElement("input", {
                disabled: userStatus !== UserStatus.LoggingIn && userStatus !== UserStatus.LogInError && userStatus !== UserStatus.LoggedIn,
...
const Weather = () => {
    const getDays = () => {
        return [{
                ...
                name: "Wed",
                temperature: N.rand(60, 80),
                weather: WeatherType.SummerTime
            }, {
                id: 4,
                name: "Thurs",
                temperature: N.rand(60, 80),
...
const Tools = () => {
    const getTools = () => {
        return [{
                ...
                id: 1,
                image: "./cale/summer-camp.jpg",
                label: "Caledonian Activits",
                name: "Summer Camp"
            }, {
                icon: "fa-solid fa-calculator-simple",
                id: 2,
                image: "./cale/summercamp.jpg",
                label: "Caledonian Activits",
                name: "Nature"
            }, {
    ...
    return (React.createElement(MenuSection, { icon: "fa-solid fa-toolbox", id: "tools-section", title: "What's Our Summer Camp?" }, getTools()));
};
const Restaurants = () => {
    const getRestaurants = () => {
        return [{
                desc: "Caledonian International School",
                id: 1,
                image: "./cale/header-school2.jpg",
                title: "Number One"
            }, {
                desc: "Caledonian International School",
                id: 2,
                ...
                desc: "Caledonian International School",
                id: 3,
                image: "./cale/class.png",
                title: "Best Education"
            }, {
                desc: "Caledonian International School",
                id: 4,
                image: "./cale/police.png",
                title: ""
            }].map((restaurant) => {
            const styles = {
...
const Movies = () => {
    const getMovies = () => {
        return [{
                ...
                image: "./cale/lib.jpg",
                title: "Caledonian Library"
            }, {
                desc: "Caledonian International School",
                id: 4,
                icon: "fa-solid fa-starship-freighter",
                image: "./cale/english.jpg",
                title: "English Club"
            }].map((movie) => {
            const styles = {
    ...
    return (React.createElement(MenuSection, { icon: "fa-solid fa-camera-movie", id: "movies-section", scrollable: true, title: "Caledonain Team" }, getMovies()));
};
...
const signIn = async (pin) => {
    try {
        const response = await fetch('http://127.0.0.1:8000/api/check-in', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
            body: JSON.stringify({
                pin: pin,  // Send the PIN instead of the user ID
            }),
        });
        if (!response.ok) {
...
const signOut = async (pin) => {
    try {
        const response = await fetch('http://127.0.0.1:8000/api/check-out', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
            body: JSON.stringify({
                pin: pin,  // Send the PIN for checkout
            }),
        });
        if (!response.ok) {
            throw new Error('Network response was not ok');
...
const App = () => {
    ...
    const handleSignOut = async () => {
        alert("dd")
        setUserStatusTo(UserStatus.LoggingOut);
    };
    ...
    return React.createElement(
        ...
        React.createElement(
            ...
            React.createElement(
                'div',
                { id: 'sign-out-button-wrapper' },
                React.createElement(UserStatusButton, {
                    icon: 'fa-solid fa-arrow-right-from-arc',
                    id: 'sign-out-button',
                    userStatus: UserStatus.LoggingOut,
...
ReactDOM.render(React.createElement(App, null), document.getElementById("cool"));